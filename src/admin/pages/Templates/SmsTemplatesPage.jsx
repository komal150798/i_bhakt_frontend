...
import styles from './SmsTemplatesPage.module.css';
...
function SmsTemplatesPage() {
  ...
  const handleSave = async () => {
    try {
      if (editingTemplate) {
        await adminApi.updateSmsTemplate(editingTemplate.id, formData);
      } else {
  ...
  const handleToggleStatus = async (templateId, currentStatus) => {
    try {
      await adminApi.toggleSmsTemplateStatus(templateId, !currentStatus);
      fetchTemplates();
    } catch (error) {
  ...
  const handleDelete = async (templateId) => {
    ...
    try {
      await adminApi.deleteSmsTemplate(templateId);
      fetchTemplates();
    } catch (error) {
  ...
  const columns = [
    ...
    { key: 'name', label: 'Name', sortable: true },
    {
      ...
      label: 'Body',
      render: (value) => (
        <div className={styles.bodyPreview} title={value}>
      ...
      <div className={styles.header}>
        ...
        {canManage && (
          <button onClick={handleAdd} className={styles.btnAdd}>
            <i className="bi bi-plus-circle"></i> Add SMS Template
          </button>
        )}
      </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setShowAddModal(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2>{editingTemplate ? 'Edit SMS Template' : 'Add SMS Template'}</h2>
